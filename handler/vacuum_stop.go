...
func NewVacuumStop(v *validator.Validate, token, vacuumID string, cfg *config.Config) *VacuumStop {
	return &VacuumStop{
		Validator: v,
		Token:     token,
		VacuumID:  vacuumID,
...
func (vs *VacuumStop) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// Send request to Home Assistant API
	haURL := fmt.Sprintf("%s/api/services/vacuum/stop", vs.Config.HomeAssistantURL())
